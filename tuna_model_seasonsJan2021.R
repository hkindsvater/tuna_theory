...
filepath <- "~/Desktop/holly_results/2021/constant/"
seasons = "NO"
...
storelimit= 0.75 #proportion of structural mass that individuals can devote to energy storage
storemin = 0.1
...
# This line, if you need it, should be after normalization, right? Or you should just skip normalization.
          ...
          for(h in 1:length(r)) { #where r is fractional allocation to reproductive effort	
              ...
              
            {
              ...
              if(Estores >= EcritL & EstoresP >= EcritLP) {
                if (Yindex > 0 )  FutureFitness <- (1-dx)*MaxF[Yindex,Lindex,p,i+1] + (dx)*MaxF[Yindex+1, Lindex, p, i+1] else FutureFitness <- (1-dx)*MaxF[Yindex+1, Lindex, p, i+1]
...
for (i in 1:(Tmax-1)) { 
  ...
  if(length(index) > 1) {
    ...
    survival[i+1] <- survival[i]*exp(-mu[size[index[1]]]-repro[index, i]*0) #this is the cumulative probability an individual survives to age i
...
data2 <- as.data.frame(t(rbind(fit_age, sizedist[1, 2:145])))
...
wtd_size <- mean(sizedist[1, 2:145]*survival[1, 2:145])

c(wtd_size, coef(fitVB)[1], coef(fitVB)[2] ) 
write.csv(survival, file=paste0(filepath,"metrics", "Temp", Temp, "f_h", round(f_h, 2),   "Kappa", round(Kappa,2),   "reprolimit", reprolimit, "Tmax", Tmax, ".csv")) 
