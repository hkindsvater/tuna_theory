...
setwd("~/Documents/tuna_theory_paper/Jeremy_machine_run_results/Sensitivity/met_coef")
 
data_files <- list.files(pattern = "\\.csv$")

repro_filenames <- data_files[((length(data_files))+1):(2*(length(data_files)))]   
state_filenames <- data_files[(2*(length(data_files))+1):(3*(length(data_files)))]
length_filenames <- data_files[1:(length(data_files))]
surv_filenames <- data_files[(3*(length(data_files))+1):(4*(length(data_files)))]
...
plot_length <- function(data, filenames) {
     ...
     matplot(t(data[,-1]), type="l", main=substr(filenames, 11, 47), col="darkgray", lwd=1.75, lty=1,  ylab="Length (cm)", ylim=c(0, 400), xlim=c(0.5, Tmax*12), xlab= "Age (years)", xaxt="n")
