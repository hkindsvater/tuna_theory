...
setwd("~/Documents/tuna_theory_paper/Tmax25/fitnessdata/fitness")
 
data_files1 <- list.files(pattern = "\\.csv$")
 
## read in files

early_filenames <- data_files1[((length(data_files1)/2)+1):(2*(length(data_files1)/2))]   
late_filenames <- data_files1[ (1: (length(data_files1)/2))]
 
earlyfit_data <- lapply(early_filenames, read.csv)
latefit_data <- lapply(late_filenames, read.csv)
 


Tmax=25
time=1:(Tmax*12)
 data<-cbind(earlyfit_data[[1]], earlyfit_data[[2]], earlyfit_data[[3]])
 data<- data[, -c(1, 3, 5)]
  matplot( (data), type="l", ylim=c(0, max(data[,1])+1e08), ylab="Lifetime Fitness",  xaxt="n", lty=1, lwd=3, col=c(1, 2, 4))
  axis(1, at = seq(0, (Tmax)*12, by=12), labels = (seq(1, Tmax+1, by=1)))
  legend("topright", legend = c("Low Food", "Med Food", "Hi Food"), lty=rep(1, 3),  col=c(1, 2, 4), bty="n")
 
  data<-cbind(latefit_data[[1]],latefit_data[[2]], latefit_data[[3]])
  data<- data[, -c(1, 3, 5)]
  matplot( (data), type="l", ylim=c(0, max(data[,1])+1e08), ylab="Lifetime Fitness",  xaxt="n", lty=1, lwd=3, col=c(1, 2, 4))
  axis(1, at = seq(0, (Tmax)*12, by=12), labels = (seq(1, Tmax+1, by=1)))
  legend("topright", legend = c("Low Food", "Med Food", "Hi Food"), lty=rep(1, 3),  col=c(1, 2, 4), bty="n")
...
setwd("~/Documents/tuna_theory_paper/Tmax25/fitnessdata")
...
plot_length <- function(data, filenames) {
  ...
  matplot(t(data[,-1]), type="l", main=substr(filenames, 23, 31), col="darkgray", lwd=1.75, lty=1,  ylab="Length (cm)", ylim=c(0, 200), xlim=c(0.5, Tmax*12), xlab= "Age (years)", xaxt="n")
  axis(1, at = seq(0, (Tmax)*12, by=12), labels = (seq(1, Tmax+1, by=1)))
  ...
  legend("topleft", legend=paste0("Lmax is ", round(data[1, 14]), " cm"), bty="n")
}
