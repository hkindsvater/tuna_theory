...
while(index <= total)
{
	if(length(suppressWarnings(system2("pgrep", "-f tuna_model_seasonsJuly2020.R", stdout=TRUE))) < limit)
	{
		argList = ""
		...
		system(paste("Rscript ~/Desktop/holly_results/code/tuna_model_seasonsJuly2020.R", argList) ,wait=FALSE)
		print(index)
		index = index + 1
	} else {
		Sys.sleep(60)
