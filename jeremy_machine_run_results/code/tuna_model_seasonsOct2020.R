...
# close(pb) #close progress bar
 ...
 set.panel(1, 4) # 2X2 matrix of plots
 
 image(optU[  , params[1], 1,   ], col=pal, ylab="Age", xlab="", las=1)
 
 
 image(optU[ , params[2] , 1,  ] , col=pal, ylab="", xlab="", las=1)
 image((1:400)*rho, 1:215, optU[  , params[3]  , 1,   ], col=pal, ylab="", xlab="", las=1)
 
  
 par(oma=c( 0,0,0,1))# 
 image.plot( zlim=c(0,1),legend.only=TRUE, smallplot=c(.1,.2,.3,.7),
             col=pal,legend.width=5, legend.shrink=.75, horizontal=F)  #make legend strip

# # 
# quartz()
# par(mfrow=c(1,3)) 	
# #image(optU[1:36, 45:100, 1,1], col=pal,   ylab="Size", xlab="Energy Stores", main="Growth, Age is 1" )
# image(optU[  , 60  , 1,   ], col=pal, ylab="Age", xlab="" ) 

# image(optU[  , 120  , 1,   ], col=pal, ylab="Age", xlab="" ) 
# image(optU[ , 210 , 1,  ] , col=pal, ylab="Age", xlab="" ) 
