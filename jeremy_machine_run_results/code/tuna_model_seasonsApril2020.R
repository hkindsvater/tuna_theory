        ...
        for (g in 1:length(u)) { #fractional placeholder (placeholder variable so we can loop over non-integers)
          ...
          for(h in 1:length(r)) { #where r is fractional allocation to reproductive effort	
              ...
              
            {
              ...
              LengthP <- L + ( (growth*Estores)/(a*scale) )^(1/3)
              ...
              threshold=L/(7+L)
               ...
               Vmat[Y,L, p, i, g, h]	<-  currentR*threshold + exp(-mu[L])*FutureFitness 
...
for (i in 1:(Tmax-1)) { 
  ...
  if(length(index) > 1) {
    ...
    nextsize <-   size[index] + ((g_allo[index,i]*state[index])/(a*scale))^(1/3)
...
write.csv(idist, file=paste0("~/Desktop/holly_results/Temp295/seasonal/03State",  "f_h", round(f_h, 2),  "Kappa", round(Kappa,2), "reprolimit", reprolimit, "Tmax", Tmax, ".csv"))
write.csv(sizedist, file=paste0("~/Desktop/holly_results/Temp295/seasonal/01Length",  "f_h", round(f_h, 2),   "Kappa", round(Kappa,2),  "reprolimit", reprolimit, "Tmax", Tmax, ".csv"))
write.csv(reproduction, file=paste0("~/Desktop/holly_results/Temp295/seasonal/02Repro",  "f_h", round(f_h, 2),   "Kappa", round(Kappa,2),  "reprolimit", reprolimit, "Tmax", Tmax, ".csv")) 
write.csv(survival, file=paste0("~/Desktop/holly_results/Temp295/seasonal/04Surv",  "f_h", round(f_h, 2),   "Kappa", round(Kappa,2),   "reprolimit", reprolimit, "Tmax", Tmax, ".csv")) 
