...
lam <- 1.95
K_c=10 #from Table 2.2, this changes with season (and is Jdensityd by Kappa)
	           ...
	           #check if this combination of allocation is viable, and this number of stores is adequate for this length: 
		          if (growth + reprod > 1  | EcritL >= Estores)   Vmat[Y,L, p, i, g, h]	<- 0 else 
		            { #given the above conditions are met calculate all states:
		              ...
		              if(YP_floor >= Yindexmax ) FutureFitness <- MaxF[Yindexmax,Lindex,p,i+1] else 
		                {
...
image.plot(1:Lmax, 1:Yindexmax, t(optU[,, 1,Tmax-1]), breaks = c(-1.5, -0.5, seq(0.1,1, by=0.1)), col=gcol,  ylab="State", xlab="Length", main="growth")       

image.plot(1:Lmax, 1:Yindexmax, t(optR[,, 1,Tmax-1]),breaks = c(-1.5, -0.5, seq(0.1,1, by=0.1)), col=gcol,  ylab="State", xlab="Length", main="reproduction")       

image.plot(1:(Tmax-1), 1:Yindexmax, t(optU[,301, 1,]),breaks = c(-1.5, -0.5, seq(0.1,1, by=0.1)), col=gcol,  ylab="State", xlab="Age", main="growth")       

image.plot(1:(Tmax-1), 1:Yindexmax, t(optR[,301, 1,]),breaks = c(-1.5, -0.5, seq(0.1,1, by=0.1)), col=gcol,  ylab="State", xlab="Age", main="reproduction")       


image.plot(1:(Tmax-1), 1:Lmax, t(optU[80, , 1,]),breaks = c(-1.5, -0.5, seq(0.1,1, by=0.1)), col=gcol,  ylab="Length", xlab="Age", main="growth")       

image.plot(1:(Tmax-1), 1:Lmax, t(optR[80, , 1,]),breaks = c(-1.5, -0.5, seq(0.1,1, by=0.1)), col=gcol,  ylab="Length", xlab="Age", main="reproduction")       
...
initialsize <- rep(20, 2) #as.integer(rnorm(nindiv, mean=50, sd=2.5))
...
z<- Jdensity*a*initialsize^3*(storemax - 0.05) #fixed initial state
...
survival[1]<-1
	...
	for (i in 1:(Tmax-1)) { 
	   ...
	   EstoresmaxL <-Wstructure*storemax*Jdensity #find the maximum jules of energy that can be stored for fish of this length  
	   ...
	   Wtotal <-  Wstores+Wstructure   #body mass
	       ...
	       if(length(index) > 1) {
		    ...
		    ((EstoresP*Yindexmax^3)/((Jdensity*a*Lmax^3)*storemax))^(1/3)
