...
# 
fit_age <- 6:180 #note we are focused only on growth from 0.5 year to 15 years of age..... 
# 
# 
#### now define functions to calculate metrics
calc_metrics <-  function(sizedata, survdata, filenames) {
   
  data1 <- (as.data.frame(t(rbind(fit_age, sizedata[1, 2:176]))))
  data2 <- na.omit(data1)
  ...
  wtd_size <- mean(data2[, 2]*data3, na.rm=TRUE)
  params <- filenames
  return(c(params,  wtd_size))
