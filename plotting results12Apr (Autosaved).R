...
setwd("~/Documents/tuna_theory/model_output/April 19")
...
plot_length <- function(data, filenames) {
     ...
     matplot(t(data[,-1]), type="l", main=c(substr(filenames, 9, 15), substr(filenames, 16, 24)), col="darkgray", lwd=1.75, lty=1,  ylab="Length (cm)", ylim=c(0, 400), xlim=c(0.5, 48), xlab= "Age (years)", xaxt="n")
...
mapply(age.length, length_data, repro_data, length_filenames)
  ...
  mortdata<- matrix(nrow=9, ncol= 64)   	 
 mort.est <- function(idata, filename) {
     ...
     m1<- lm(log(alive_at_age) ~ age) 	
 	 ...
 	 mort_dist <- mapply(mort.est, state_data, state_filenames)

# quartz()
 # par(mfrow=c(2, 3))
# mortdata<-mapply(mort.est, state_data, state_filenames)
 
# matplot(  (mortdata[,13:15 ]), type="l", lty=1, lwd=3, xaxt="n", xlim=c(1, 56), xlab="Age", ylab="Number alive", col=c(1, 3, 4))
# axis(1, at = seq(0, 60, by=4), labels = (seq(1, 16, by=1)))

 # matplot( log (mortdata[,13:15 ]), type="l", lty=1, lwd=3, xaxt="n", xlim=c(1, 56), xlab="Age", ylab="Ln number alive", col=c(1, 3, 4))
 # axis(1, at = seq(0, 60, by=4), labels = (seq(1, 16, by=1)))
