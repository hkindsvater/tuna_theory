...
setwd("~/Documents/tuna_theory_paper/jeremy_machine_run_results/Rlim.3/constant/")
...
food_tab <- as.numeric(substr(results[1, ], 16, 19))*12 
 tabdata<- cbind(seasonal, temp_tab, mort_tab, food_tab, round(as.numeric(results[2, ])), round(as.numeric(results[3, ]),2), round(as.numeric(results[4, ])), round(as.numeric(results[5, ]), 3), round(as.numeric(RE, 3)))
 colnames(tabdata) <- c("Seasonality", "Temp", "Predator_fullness", "Richness", "Max_length", "Weighted_mean_length", "Linf", "von_Bert_K", "Mean_RE")
 tabdata


 ###now repeat this for seasonal results 
 setwd("~/Documents/tuna_theory_paper/jeremy_machine_run_results/Rlim.3/seasonal/")
...
food_tab <- as.numeric(substr(results[1, ], 16, 19))*18  
 ...
 tabdata2<- cbind(seasonal, temp_tab, mort_tab, food_tab, round(as.numeric(results[2, ])), round(as.numeric(results[3, ]),2), round(as.numeric(results[4, ])), round(as.numeric(results[5, ]), 3), round(as.numeric(RE, 3)))
 colnames(tabdata2) <- c("Seasonality", "Temp", "Predator_fullness", "Richness", "Max_length", "Weighted_mean_length", "Linf", "von_Bert_K", "Mean_RE")
 tabdata2
