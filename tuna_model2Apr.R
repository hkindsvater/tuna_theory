...
# library(fields)
 setwd("/Users/hollykindsvater/Dropbox")
...
idist[, -Tmax]<-ifelse(idist[, -Tmax]>0,  idist[, -Tmax], NA)
   ...
   write.csv(idist, file=paste0("results3Apr/03State",  "Temp", Temp, "c1", c1, "Kappa", Kappa,  ".csv"))

write.csv(sizedist, file=paste0("results3Apr/01Length",  "Temp", Temp, "c1", c1, "Kappa", Kappa,  ".csv"))
write.csv(reproduction, file=paste0("results3Apr/02Repro",  "Temp", Temp, "c1",  c1, "Kappa", Kappa,   ".csv")) 
