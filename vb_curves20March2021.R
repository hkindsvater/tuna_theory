...
return(wtd_rep)
 ...
 storelim=rep(0.60, 17)
 seasonality = rep("constant", 17)
...
#food_tab <- as.numeric(substr(results[1, ], 16, 19))*18  
 tabdata<- cbind(seasonality, temp_tab, mort_tab, food_tab, round(as.numeric(results[2, ])), round(as.numeric(results[3, ]),2), round(as.numeric(results[4, ])), round(as.numeric(results[5, ]), 3), round(as.numeric(RE, 3)))
...
setwd("~/Documents/tuna_theory_paper/jeremy_machine_run_results/finegrid/constant/storelim0.75")
...
storelim<- rep(0.6, 16) 
seasonality<-(rep("seasonal", 16))
temp_tab <- as.numeric(substr(results[1, ], 1, 3))
mort_tab <- as.numeric(substr(results[1, ], 7, 10))*12
food_tab <- as.numeric(substr(results[1, ], 16, 19))*18  
 #NOTE for the seasonal results I shifted the average richness of the ecosystem.  
 tabdata2<- cbind(seasonality, temp_tab, mort_tab, food_tab, round(as.numeric(results[2, ])), 
