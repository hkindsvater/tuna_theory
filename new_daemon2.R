...
name = "results3Apr"
newDir = paste("mkdir /Users/hollykindsvater/Dropbox/", name, sep="") 
system(newDir)
...
while(index <= total)
{
	if(length(suppressWarnings(system2("pgrep", "-f tuna_model2Apr.R", stdout=TRUE))) < limit)
	{
		argList = ""
		...
		system(paste("Rscript ~/Dropbox/tuna_model2Apr.R", argList) ,wait=FALSE)
		print(index)
		index = index + 1
	} else {
		Sys.sleep(60)
