...
setwd("~/Documents/tuna_theory_paper/jeremy_machine_run_results/finegrid/constant/storelim0.6/phi_sens")
...
return(wtd_rep)
  ...
  phi_pindex <- regexpr("phi_p", length_filenames) 
 ...
 phi_aindex <- regexpr("phi_a", length_filenames) 
 phi_ptab <- as.numeric(substr(length_filenames, phi_pindex+ 5[1], phi_pindex+8[1]))
 phi_atab <- as.numeric(substr(length_filenames, phi_aindex+5[1], phi_aindex+7[1])) 
mort_tab <- as.numeric(substr(length_filenames,  mort_index+3[1],   mort_index+6[1]))*12

food_tab <- as.numeric(substr(length_filenames, kappa_index+5[1],   kappa_index+9[1]))*12 
 tabdata<- cbind(storelim, phi_ptab, phi_atab, mort_tab, food_tab, round(as.numeric(results[2, ])), round(as.numeric(results[3, ]),2), 
                 round(as.numeric(results[4, ])), round(as.numeric(results[5, ]), 3))
 colnames(tabdata) <- c("Store_limit", "phi_a", "phi_p" "Predator_fullness", "Richness", "Max_length", "Weighted_mean_length", "Linf", "von_Bert_K")
 tabdata
